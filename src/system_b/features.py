...
from shared.garch import ewma_sigma, fit_garch
from shared.regime import RegimeReading, market_index_returns
from shared.schema import ItemMeta, SourceStatus
from shared.signal_bus import BusSignal, attention_for_item
...
def _structural(meta: ItemMeta | None, cfg_sel: dict, priority_weapons: list[str],
    ...
    lo, hi = cfg_sel.get("supply_sweet_spot", [2000, 10000])
    blo, bhi = cfg_sel.get("supply_broad", [10000, 30000])
    s = meta.supply
    if lo <= s <= hi:
        supply_band = 1.0
    elif blo <= s <= bhi:
