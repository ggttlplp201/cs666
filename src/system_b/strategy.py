...
from shared_b.breaks import cusum_break
from shared_b.data import PanelView
from shared_b.journal import Journal
from shared_b.ledger import Ledger
from shared_b.regime import RegimeReading, classify_regime, market_index_returns
from shared_b.schema import Order, Regime, Side
from shared_b.signal_bus import NullBus, SignalBus, confirmed_events_for_item
...
class PositionalStrategy:
    ...
    def _exits(
        ...
        # distribution-shape exit gating: a "high peak" volume profile right
        # after a flat-base entry is often the accumulation base itself, so the
        # rule can demand real evidence of distribution before crystallizing a
        # loss (a meaningful drawdown / seasoned lot / falling cost zone).
